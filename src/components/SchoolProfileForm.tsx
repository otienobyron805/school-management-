...
import { Save, Sparkles, Building2, Phone, Target, Calendar, Info } from 'lucide-react';
...
export default function SchoolProfileForm() {
  ...
  const handleChange = (field: keyof SchoolProfile, value: string) => {
    ...
    setProfile(updated);
    // Auto-save to localStorage so data isn't lost
    saveSchoolProfile(updated);
    // Note: Global storage event is NOT dispatched here to prevent App.tsx re-renders during typing
  };
  ...
  return (
    <form onSubmit={handleSaveProfile} className="space-y-8 pb-16 relative">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField label="Term Dates" value={profile.termDates} onChangeField={(v) => handleChange('termDates', v)} disabled={isParent} />
          <FormField label="Academic Calendar Description" type="textarea" value={profile.academicCalendar} onChangeField={(v) => handleChange('academicCalendar', v)} disabled={isParent} />
          <FormField label="Headteacher / Principal Name" value={profile.principalName} onChangeField={(v) => handleChange('principalName', v)} disabled={isParent} />
          <FormField label="Appointment Date" type="date" value={profile.appointmentDate} onChangeField={(v) => handleChange('appointmentDate', v)} disabled={isParent} />
        </div>
      </div>
